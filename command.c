...
noreturn void external_command(char **argv) {
  ...
  if (!index(argv[0], '/') && path) {
    ...
    size_t current_position_in_path = 0;
    // Przechodzimy po kolejnych ścieżkach w PATH
    while (current_position_in_path < path_size) {
      // szukamy końca obecnej ścieżki (znak : lub koniec PATH)
      size_t current_path_length =
        strcspn(path + current_position_in_path, ":");
      char *current_path =
        strndup(path + current_position_in_path, current_path_length);
      strapp(&current_path, "/");
      strapp(&current_path, argv[0]);
      (void)execve(current_path, argv, environ);
      // zwiększamy pozycję o długość obecnej ściezki i pomijamy również znak :
      current_position_in_path += current_path_length + 1;
      free(current_path);
    }
  } else {
